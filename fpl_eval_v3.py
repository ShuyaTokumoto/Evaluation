...
def prepare(df_raw, team_map):
    df = df_raw.copy()
    df["player_name"] = df["web_name"] if "web_name" in df.columns else pd.Series("", index=df.index, dtype=str)
    df["position"]    = (df["element_type"] if "element_type" in df.columns else pd.Series(0, index=df.index)).map(POS_MAP).fillna("UNK")
    df["team_name"]   = (df["team"] if "team" in df.columns else pd.Series(0, index=df.index)).map(team_map).fillna("Unknown")
    for c in [
